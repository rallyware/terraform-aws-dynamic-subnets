...
variable "nat_instance_ami_architecture" {
  type        = string
  description = "NAT Instance AMI architecture"
  default     = "arm64"
}

variable "nat_instance_ami_lock" {
  type        = bool
  description = "Whether to lock NAT Instance AMI vesrion"
  default     = true
}
...
variable "nat_instance_cloudwatch_metric_alarm" {
  type = object({
  ...
  default = {
    ...
    statistic           = "Maximum"
    threshold           = 1
    action              = "action/actions/AWS_EC2.InstanceId.Reboot/1.0"
  }
  description = "NAT Instance CloudWatch Metric Alarm resource config"
}
